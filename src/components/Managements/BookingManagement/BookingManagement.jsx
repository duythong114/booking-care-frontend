...
const BookingManagement = () => {
  ...
  const isGettingAllBookings = useSelector(
    (state) => state.booking.isGettingAllBookings
  );
  const isDeletingBooking = useSelector(
    (state) => state.booking.isDeletingBooking
  );
  const isSearchingBooking = useSelector(
    (state) => state.booking.isSearchingBooking
  );
  ...
  const isGettingAvailable = useSelector(
    (state) => state.booking.isGettingAvailable
  );
  const isCreatingBooking = useSelector(
    (state) => state.booking.isCreatingBooking
  );
  const availableBookingList = useSelector(
    (state) => state.booking.availableBookingList
  );
  ...
  useEffect(() => {
    if (availableBookingList) {
      setMorningAppointments(
        availableBookingList.filter(
          (appointment) => appointment.appointmentTime === "Morning"
        )
      );
      setAfternoonAppointments(
        availableBookingList.filter(
          (appointment) => appointment.appointmentTime === "Afternoon"
        )
      );
    }
    ...
    if (
      ...
      try {
        const response = await dispatch(
          createBooking(selectedAppointment)
        ).unwrap();
        if (response?.message) {
          handleToggleBookingModal();
          toast.success(response.message);
  ...
  return (
    <div className="booking-mana-container">
      ...
      <div className="booking-header">
        ...
        <button onClick={() => handleBookingBtn()} className="btn btn-primary">
          Booking
      ...
      <ModalComponent
        ...
        body={
          <div className="booking-container">
            ...
            <select
              onChange={(e) =>
                handleAppointmentChange(e.target.value, "Morning")
              }
            >
            ...
            <select
              onChange={(e) =>
                handleAppointmentChange(e.target.value, "Afternoon")
              }
            >
